...
void read_settings(const std::string& fname, Settings &settings) {
    ...
    settings.t_max = 0.0001;
    settings.dt = 1e-5;
    settings.dt_grow = 1.0;
    settings.particles.ensemble = 10000;
    settings.outputs.output_particle_positions = true;
    settings.outputs.echo_settings = true;
